...
import tags from 'C:/Playwrite_js/pw-practice-app/test_data/tags.json';

test.beforeEach(async ({page}) =>{
    await page.route('*/**/api/tags', async route => { // matches any pattern of the URL
        await route.fulfill({
...
test('has title', async ({page}) => {
    await page.route('*/**/api/articles*', async route => {
        ...
        responceBody.articles[0].title = "This is a MOCK test title"
...
test('create article', async({page, request}) => {
    ...
    await page.getByRole('textbox', {name: 'Article Title'}).fill('This is a test title2')
    await page.getByRole('textbox', {name: "What's this article about?"}).fill('This is a test description2')
    await page.getByRole('textbox', {name: 'Write your article (in markdown)'}).fill('This is a test body2')
    ...
    await expect(page.locator('app-article-page h1')).toContainText("This is a test title2")
